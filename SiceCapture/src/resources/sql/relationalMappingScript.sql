...
CREATE TABLE IF NOT EXISTS `mydb`.`Image` (
  ...
  PRIMARY KEY (`idImage`, `fk_expedient`, `fk_document`, `fk_client`),
  INDEX `fk_Image_Expedient1_idx` (`fk_expedient` ASC),
  INDEX `fk_Image_Document1_idx` (`fk_document` ASC),
  INDEX `fk_Image_Client1_idx` (`fk_client` ASC),
  CONSTRAINT `fk_Image_Expedient1`
    FOREIGN KEY (`fk_expedient`)
    REFERENCES `mydb`.`Expedient` (`idExpedient`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Image_Document1`
    FOREIGN KEY (`fk_document`)
    REFERENCES `mydb`.`Document` (`idDocument`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Image_Client1`
    FOREIGN KEY (`fk_client`)
    REFERENCES `mydb`.`Client` (`idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
